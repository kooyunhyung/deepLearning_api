...
@router.post(f"{STUDENTS_ROUTE}/insert", response_model=schemas.Shirts)
def create_student(student: schemas.Shirts, db: Session = Depends(get_db)):
    return crud.create_student(db, student)


@router.get(f"{STUDENTS_ROUTE}", response_model=List[schemas.Shirts])
def read_student(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    students = crud.get_students(db, skip=skip, limit=limit)
    return students


@router.put(f"{STUDENTS_ROUTE}/update/{{student_id}}", response_model=schemas.Shirts)
